...
#' @export
  ...
  nodal_vote <- function(x, resolve_ties = FALSE) {
    ...
    max_list <-
      map(rownames(x), function(row) set_names(x[row, ], colnames(x)))

    max_list <- set_names(max_list, rownames(x))

    max_list <- map(max_list, ~.x[.x == max(.x)])

    if(!resolve_ties) {

      assign_tbl <- map_dfr(max_list,
                            ~tibble(class = names(.x)[1],
                                    best_p = .x[1]))
    ...
    relocate(assign_tbl, observation)
